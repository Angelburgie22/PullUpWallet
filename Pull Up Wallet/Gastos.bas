...
Sub Globals
	...
	Type IndiceGasto(indiceElemento As Int, rowid As Int)
	Type Gasto(rowid As Int, nombre As String, monto As Int, fecha As String, descripcion As String, color As Int)
...
Sub Activity_Create(FirstTime As Boolean)
	...
	'https://www.b4x.com/android/forum/threads/hide-scrollbar-in-scrollview.56894/
	...
	'Itera sobre todos los registros y guarda los campos del registro actual en un objeto de tipo Gasto, luego
	...
	If nRegistros > 0 Then
		...
		For id = 0 To nRegistros - 1
			...
			Dim datosGasto As Gasto
			datosGasto.rowid = cur.GetInt("id_gasto")
			datosGasto.nombre = cur.GetString("nombre")
			datosGasto.monto = cur.GetInt("monto")
			datosGasto.descripcion = cur.GetString("descripcion")
			datosGasto.fecha = cur.GetString("fecha_registro")
			datosGasto.color = cur.GetInt("color")
			'Se envian los argumentos necesarios para crear el componente de la lista
			creaObjetoLista(id, top, datosGasto)
	...
	Else
		...
		lblMensaje.Text = cs.Initialize.Color(Colors.Black).Alignment("ALIGN_CENTER").Typeface(Typeface.MATERIALICONS).Size(50).Append(Chr(0xE89C) & CRLF).Typeface(Typeface.DEFAULT).Size(18).Append("Está muy vacío aquí adentro." & CRLF).Append("Agrega un gasto desde el menú principal.").PopAll
	...
	'----------------------------------------------------------------------------
	'Eliminacion de un gasto fijo
	sumaEgresosAntigua = 0
	montoSeleccionado = 0
	sumaEgresosActual = 0
End Sub
...
Sub creaObjetoLista(indice As Int, top As Int, datos As Gasto)
	Dim pnlGasto As Panel
	...
	'Panel contenedor
	pnlGasto.Initialize("pnlGasto")
	pnlGasto.Top = top
	pnlGasto.Background = cd
	pnlGasto.Width = anchoDispositivo * 0.9
	pnlGasto.Height = alturaDispositivo * 0.14
	pnlGasto.Left = (anchoDispositivo - pnlGasto.Width) / 2
	...
	lblNombre.Height = pnlGasto.Height * 0.3
	lblNombre.Width = pnlGasto.Width * 0.9
	lblNombre.Top = 0
	lblNombre.Left = pnlGasto.Width * 0.05
	lblNombre.Gravity = Gravity.CENTER_VERTICAL
	pnlGasto.AddView(lblNombre, lblNombre.Left, lblNombre.Top, lblNombre.Width, lblNombre.Height)
	...
	lblDescripcion.Height = pnlGasto.Height * 0.3
	lblDescripcion.Width = pnlGasto.Width * 0.9
	lblDescripcion.Top = lblNombre.Height
	lblDescripcion.Left = lblNombre.Left
	lblDescripcion.Gravity = Gravity.CENTER_VERTICAL
	pnlGasto.AddView(lblDescripcion, lblDescripcion.Left, lblDescripcion.Top, lblDescripcion.Width, lblDescripcion.Height)
	'Etiqueta de la fecha
	lblFecha.Initialize("lblFecha")
	lblFecha.Text = datos.fecha
	lblFecha.TextSize = 16
	lblFecha.TextColor = Colors.DarkGray
	lblFecha.Height = pnlGasto.Height * 0.4
	lblFecha.Width = pnlGasto.Width * 0.9
	...
	pnlGasto.AddView(lblFecha, lblFecha.Left, lblFecha.Top, lblFecha.Width, lblFecha.Height)
	'Etiqueta del monto
	...
	lblMonto.Height = pnlGasto.Height * 0.4
	lblMonto.Text = "$" & datos.monto
	lblMonto.Top = lblFecha.Top
	lblMonto.Left = pnlGasto.Width - lblMonto.Width - lblNombre.Left
	lblMonto.Gravity = Gravity.CENTER_VERTICAL
	pnlGasto.AddView(lblMonto, lblMonto.Left, lblMonto.Top, lblMonto.Width, lblMonto.Height)
	...
	pnlGasto.AddView(lblOpciones, lblOpciones.Left, lblOpciones.Top, lblOpciones.Width, lblOpciones.Height)
	...
	pnlOpciones.Height = pnlGasto.Height * 0.8
	pnlOpciones.Width = pnlGasto.Height
	pnlOpciones.Top = lblOpciones.Top
	pnlOpciones.Left = lblOpciones.Left - lblOpciones.Width * 2 - pnlOpciones.Width
	pnlOpciones.Visible = False
	pnlGasto.AddView(pnlOpciones, pnlOpciones.Left, pnlOpciones.Top, pnlOpciones.Width, pnlOpciones.Height)
	...
	pnlDeslizable.AddView(pnlGasto, pnlGasto.Left, top, pnlGasto.Width, pnlGasto.Height)
	
	
	'Se crea este objeto de tipo Indice para guardar solo el indice del objeto actual
	Dim referencia As IndiceGasto
	...
	Dim referencia2 As IndiceGasto
...
Sub btnEditar_Click
	...
	CallSubDelayed2(editarGasto, "consultaAEditar", idRegistro)
	Activity.Finish
End Sub
...
Sub btnEliminar_Click
	...
	resultado = Msgbox2("¿Estás seguro?", "Eliminar gasto", "Sí", "", "No", Null)
	If (resultado = DialogResponse.POSITIVE) Then
		...
		sumaEgresosActual = sumaEgresosAntigua - montoSeleccionado
		...
		sql.ExecNonQuery("DELETE FROM gastos WHERE id_gasto = '" & idRegistro & "'")
