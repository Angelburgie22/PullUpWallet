...
Sub Activity_Create(FirstTime As Boolean)
	...
	btnOperacionSuscripcion.Text = "Editar suscripción"
...
'Valida el rellenado de los campos, obtiene el objeto de tipo RegistroSuscripcion de asignacionCampos y realiza la actualización 
...
Private Sub btnOperacionSuscripcion_Click
	...
	If(validaciones = True) Then
		...
		Dim suscripcionRegistro As RegistroSuscripcion
		suscripcionRegistro = asignacionCampos
		
		'Sentencia para realizar una actualización en la tabla suscripciones
		sql.ExecNonQuery2("UPDATE suscripciones SET monto = ?, nombre = ?, descripcion = ?, periodo = ?, color = ? WHERE id_suscripcion = '" & idRegistro & "'", Array As Object(suscripcionRegistro.monto, suscripcionRegistro.nombre, suscripcionRegistro.descripcion, suscripcionRegistro.periodo, suscripcionRegistro.color))

		ToastMessageShow("Suscripción editada", True)
...
'Realiza la consulta de los datos del registro a editar y los guarda en las propiedades de un objeto de tipo RegistroSuscripcion
Sub consultaAEditar(rowid As Int)
	Dim suscripcion As RegistroSuscripcion
	idRegistro = rowid

	cur = sql.ExecQuery("SELECT * FROM suscripciones WHERE id_suscripcion = '" & idRegistro & "'")
	If(cur.RowCount > 0) Then
		cur.Position = 0
		suscripcion.nombre = cur.GetString("nombre")
		suscripcion.monto = cur.GetInt("monto")
		suscripcion.descripcion = cur.GetString("descripcion")
		suscripcion.periodo = cur.GetInt("periodo")
		suscripcion.color = cur.GetString("color")
		
		reasignarCamposAEditar(suscripcion)
	End If
End Sub

'Asigna los campos del registro a editar, en los views
Sub reasignarCamposAEditar(registro As RegistroSuscripcion)
